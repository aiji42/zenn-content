...
`useFormSubmit`が返却する`state`の一覧は`idle | submitting　| routing | error | success`となっています。
...
しかし、サーバサイドでバリデーションを挟んで、エラーケースではエラーメッセージをpropsで返し、今いるページにとどまらせてメッセージを表示する実装を考えてみてください。
ピュアなform elementによるサブミットでは、最終的には同じ結果が得られますが、強制的にフルページリロードが挟まってしまいます。
