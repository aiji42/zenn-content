...
この記事では、はじめに私の感じたRemixの「ここが優れている」という点と、それらをNext.jsに取り入れる方法を紹介します。
...
Next.jsはリクエストのハンドラが一つです。一方、RemixはGETを受ける`loader`と、GET以外のPOSTやDELETEなどを受ける`action`の2つに別れます。 

Next.jsのgetServerSidePropsでPOSTリクエストを受ける場合、Bodyのパース処理を自身で書く必要があります。そのため、getServerSidePropsではなく[API Routes](https://nextjs.org/docs/api-routes/introduction) を利用し、ミューテーション用のエンドポイントを増やすことが一般的です。

Remixのactionで取り扱うrequestは拡張されているため、次のように直接FormDataを参照することが可能です。
...
**Remixは同一のエンドポイントでコンテントリクエストとミューテーションを行うことをが可能で**、これはRails等の元来のWebフレームワークのようなRESTfulなエンドポイント構成です。これによりコントローラロジックの凝縮が容易になります。
Next.jsでも同じように構成可能ですが、前述の通り一定の拡張が必要になります。
...
ものすごく壮大な話に聞こえますが、実際にライブラリを一つ利用するだけです。
