...
next-runtimeの特筆すべき機能下記のとおりです。
- getServerSidePropsを拡張し、GETとPOST(それ以外のメソッドにも対応可)とでロジックを分離させる
  - ミューテーションの処理をAPI Routeを利用せずに書くことができる
  - ページ表示用エンドポイントでミューテーションリクエストを受けることができる
- リクエスト時の`accept Header`と`content-type Header`に応じて、応答のコンテントタイプを自動的に切り替えてくれる
...
下記は公式ドキュメントのサンプルコードで、サブミット中にローディングのコンポネントを表示しているだけですが、  
useFormSubmitの返り値には`values`が含まれているため、そのデータを利用して楽観的UIを構築することが可能です。
...
ステートの一覧は`idle | submitting　| routing | error | success`となっています。
ここで注目していただきたいのが`routing`です。このステートがあるのは、サブミット先のエンドポイントがリダイレクトを返却した場合に、Formコンポネントが自動的にリダイレクト先にページをナビゲートしてくれる機能を持っているためです。
もちろん、ピュアなform elementを利用してブラウザネイティブに同じエンドポイントにリクエストしても最終的には同じ結果が得られます。  
しかし、サーバサイドでバリデーションを挟んで、バリデーションエラーが起きた場合にはエラーpropsを返し、今いるページにとどまらせてエラーを表示する実装を考えてみてください。
ピュアなform elementによるサブミットでは、強制的にページがリロードされてしまいます。
Formコンポネントは`event.preventDefault`でデフォルトの挙動を停止し、内部でfetch関数による通信に書き換えることで、ナビゲートすべきか、とどまらせて再レンダリングすべきかをコントロールしています。
