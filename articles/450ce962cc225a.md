...
:::message
個人的にはパターン3がおすすめですので、時間がない方はパターン3のみ読んでいただければ十分かと思います。
:::
...
`_app` で、その `getAccessControl` を取り出して実行することで、アクセスコントロールの制御ルールを得て、それに従って処理をするという流れです。
そうすることで、アクセスコントロールの制御構文そのものは、各ページに閉じることになるため、関心分離が行えており、処理の複雑化に伴う `_app` のバンドルサイズ増加も防止できます。
...
もし、特定パスに対しての非認証ユーザのアクセスをブロック・リダイレクトするだけでよいのであれば、こちらのパッケージもおすすめです。
firebase auth, cognito, auth0 などの認証プロバイダだけでなくIPレンジでの制御も可能です。
https://github.com/aiji42/next-fortress

(スターやコントリビュートお待ちしています。)
