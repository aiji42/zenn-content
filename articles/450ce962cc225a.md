...
例えば、一般ユーザ向けのページと、登録済みユーザ向けのマイページを持つケースを考えてみます。
...
:::message
個人的には中規模以上のプロジェクトではパターン3をおすすめします。
:::
...
例えば、`/mypage/a` `/mypage/b` `/mypage/c` のように複数ページある場合や、新しくページを追加する場合に、アクセスコントロールの記述が漏れてしまう可能性があります。
