...
Storybook、Ladle、Histoire に関しては、ビルド結果のディレクトリ(もしくはサーバのURL)を設定ファイル上で記述して、Lost Pixelを実行すると、自動的にHTTPサーバが立ち上がって各シナリオ(ストーリー)を撮影してくれます。  

Page shots は設定ファイルにURLのリストを渡すことで、先に上げたStorybookなどのカタログ以外のWebページなどのキャプチャが可能です。  
先述の通り、設定ファイル自体がTS/JSで記述可能なので、URLのリストを動的に作ることも可能です。
さらに、Custom shotsは、事前にCypressやPlaywrightで撮影した画像を、そのまま比較処理にかけることができるモードです。
...
Lost Pixelは、VRTを行うためのツールで、キャプチャの撮影と差分の検知を一つのプロセスで行うことができるオープンソースライブラリです。
下記のような特徴を持っています。
