...
[Remix](https://remix.run/)の使用経験のある方は、「ライブラリを追加したらビルドできなくなったぞ？」という問題に出会ったことがあるでしょう。
...
```
デフォルトでは、`remix setup`の記述がされているはずですので、その後で実行されるように記述しましょう。
...
### remixのランタイムをCloudflare Workersにした場合にビルドが失敗する問題に対処する
...
本来、`browser > module > main`の優先位順位で解決していくのが望ましく、すでにその[PR](https://github.com/remix-run/remix/pull/2076)は上がっていまが、残念ながらまだマージされていません。
という事で、`remix-esbuild-override`を使用して、バグが修正されるまでの間をしのぎましょう。
